"""
Method 1: Reduce halo by using smaller r ONLY for transmission refinement.
Other box filters keep large r to maintain color stability.
...
def dehaze_m1(I_bgr, omega_base, rgb_scale=(0.80, 0.93, 1.05),
              r_base=60, r_trans=10, t0=0.1,
              alpha_clear=0.30, power=3, omega_boost=0.10,
              sat_scale=0.04, sat_power=3, max_dev=0.05):
    """
    Only transmission refinement uses smaller r_trans.
    Other 3 box filter locations keep r_base (large) for color stability.
    ...
    for c, scale_c in enumerate(rgb_scale):
        ...
        t_c_ref = box_smooth(t_c_raw, r=r_trans)    # <-- smaller r here only
        t_c = np.maximum(t_c_ref, t0)
        t_channels.append(t_c)

    # Cross-channel clamp: prevent per-channel t from diverging
    ...
    J = np.zeros_like(I)
    for c, scale_c in enumerate(rgb_scale):
        dc_smooth = box_smooth(dc_channels[c], r=r_base)       # keep large r
...
if __name__ == "__main__":
    ...
    cv2.imwrite(os.path.join(OUT_DIR, '18_m0_original_r60.png'), J_orig)

    print("=== Method 1: Smaller r + Cross-Channel Clamp ===")
    ...
    best_cfg = (10, 0.05)
    best_ssim = 0
    for r_t in [5, 8, 10, 12, 15, 20]:
        for mdev in [0.03, 0.04, 0.05, 0.06, 0.08, 0.10]:
            J = dehaze_m1(I_bgr, omega_base, r_trans=r_t, max_dev=mdev)
            p = psnr(J, G)
            s = ssim_val(J, G)
            dp = p - p_orig
    ...
    for r_t in range(max(1, br - 3), br + 4):
        for mdev in [max(0.01, bmd - 0.02), max(0.01, bmd - 0.01), bmd,
                     bmd + 0.01, bmd + 0.02]:
            J = dehaze_m1(I_bgr, omega_base, r_trans=r_t, max_dev=mdev)
            p = psnr(J, G)
            if p > best_psnr:
    ...
    J_best = dehaze_m1(I_bgr, omega_base, r_trans=r_t, max_dev=mdev)
    out_path = os.path.join(OUT_DIR, '18_m1_smallr.png')
