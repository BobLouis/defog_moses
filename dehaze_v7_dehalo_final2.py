...
"""
Per-channel DCP + Dual Local Atmospheric Light — NO Guided Filter (Hardware Dehalo Version)
Final 2: Adaptive edge-based clamp only — simpler hardware, better img18

Strategy: Keep original large-r transmission everywhere.
Only add edge-adaptive cross-channel clamp at strong edges to reduce color fringing.
No extra box filter needed (simpler than Final 1).

Changes vs baseline (dehaze_v7_3type.py / dehaze_no_gf):
  1) Edge detection: Sobel gradient on grayscale input
     -> Hardware: 3x3 convolution (Gx, Gy), sqrt(Gx^2+Gy^2), threshold+ramp
  2) At edges only: cross-channel clamp (max_dev=0.05)
     -> Hardware: mean(t_R,t_G,t_B) + comparator + mux
  3) In flat areas: identical to original dehaze_v7_3type (no change at all)

Simpler than Final 1 (no extra small box filter), better edge artifact fix.
OHaze result: avg PSNR +0.002 vs baseline, img18 PSNR +0.044
"""
import numpy as np
import cv2
...
def dehaze_no_gf(I_bgr, omega_base, rgb_scale=(0.80, 0.93, 1.05),
                 r=60, t0=0.1,
                 alpha_clear=0.30, power=3, omega_boost=0.10,
                 sat_scale=0.04, sat_power=3,
                 max_dev=0.05, edge_thresh=40, edge_sigma=20):
    """
    Per-channel DCP + Dual Local A + Omega Boost — NO guided filter.
    Adaptive dehalo: at strong edges -> cross-channel clamp only.
    In flat areas -> identical to original (no change).
    """
    ...
    edge_w = np.clip((grad - edge_thresh) / (edge_sigma + eps), 0, 1).astype(np.float32)

    # --- Compute per-channel transmissions (same as original) ---
    t_channels = []
    dc_channels = []
    J = np.zeros_like(I)
    for c, scale_c in enumerate(rgb_scale):
        ...
        t_c_ref = box_smooth(t_c_raw, r=r)            # <-- same as original (large r)
        t_c = np.maximum(t_c_ref, t0)
        t_channels.append(t_c)

    # --- Adaptive cross-channel clamp (ONLY at edges) ---
    t_mean = np.mean(t_channels, axis=0)
    for c in range(3):
        t_clamped = np.clip(t_channels[c], t_mean - max_dev, t_mean + max_dev)
        # Blend: flat areas keep original t, edges get clamped t
        t_channels[c] = (1.0 - edge_w) * t_channels[c] + edge_w * t_clamped

    # --- Recover scene ---
    for c in range(3):
        dc_smooth = box_smooth(dc_channels[c], r=r)
...
if __name__ == "__main__":
    ...
    defog_version = "dehaze_v7_dehalo_final2"
    ...
    def main(dataset):
        ...
        for hazy_path in hazy_files:
            ...
            try:
                ...
                omega_base = REFINED_OMEGA.get(fname, DEFAULT_OMEGA)
    ...
    # ========== Summary ==========
    print(f"\n\n{'='*70}")
    print(f"Dehalo Final2 (Adaptive clamp-only) Summary vs Targets")
    print(f"{'='*70}")
