...
def compute_psi_map_method3(H, A, psi_min=0.50, psi_max=1.38, buffer_size=8, epsilon=1e-6,
...
def defog_img(hazy_image, psi_min=0.50, psi_max=1.38, t0=0.25, window_size=8, buffer_size=8, epsilon=1e-6):
    """
    基於 proposed_v5 方法 + 動態 PSI 調整 (方法三：混合法)
    ...
    # ========== 計算 PSI Map (方法三：混合法) ==========
    psi_map = compute_psi_map_method3(H, A, psi_min, psi_max, buffer_size, epsilon)
...
if __name__ == "__main__":
    ...
    def save_psi_heatmap(psi_map, output_path, psi_min=0.50, psi_max=1.38):
        """
        ...
        plt.title('PSI Map - Fog Density Heatmap (Method 3: Hybrid)')
        plt.axis('off')
        plt.tight_layout()
