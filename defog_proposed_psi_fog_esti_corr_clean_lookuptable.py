...
def predict_psi(image):
	...
	# FogScore range: 0-100
	lut_indices = np.linspace(0, 100, lookup_table_size)
	lut_values = 0.011099 * lut_indices + 0.746386
	lut_values = np.clip(lut_values, 0.7, 1.2)
	
	# 使用 Lookup Table 查詢 BestPsi
	# 將 fog_score 映射到 LUT 索引 (0 ~ lookup_table_size-1)
	lut_index = int((fog_score / 100.0) * (lookup_table_size - 1))
	lut_index = np.clip(lut_index, 0, lookup_table_size - 1)
	
	BestPsi = lut_values[lut_index]
	return BestPsi
